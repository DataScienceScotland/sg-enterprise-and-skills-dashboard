...
ES_thirdPage_UI <- function(id, variableName) {
  ...
  if (length(disag) > 1) {disag <- c(disag, "All Available Breakdowns")}
  ...
  if (!(toupper(variableName) %in% c("ENTREPRENEURIAL ACTIVITY","PRODUCTIVITY","SPEND ON RESEARCH AND DEVELOPMENT"))) {
    ...
    page <- tabsetPanel(
      tabPanel(title = div("Trend Data", class = "thirdPageHeader"),
      ...
      tabPanel(title = "International Data",
               column(12,
                      p(
                        paste(
                          variableName,
                      ...
                      p(paste("You can hover over the chart to get more information about these countries performance.",
...
ES_thirdPage_Server <- function(input, output, session, dataset, ID_name, internationalData = NA, 
  ...
  output$regionalDropDown <- renderUI({
    ...
    region_list <- dataRegion %>% pull(Disaggregation) %>% unique() %>% sort()
  ...
  output$equalityDropDown <- renderUI({
    ...
    equal_list <- dataRegion %>% pull(Disaggregation) %>% unique() %>% sort()
  ...
  breakdownFilter <- reactive({
    ...
    if (length(input$sidebarRadio) > 0) {
      if ((input$sidebarRadio %in% c("Scotland", "All Available Breakdowns"))) {gather1 <- c(gather1, "Scotland")}
      if ((input$sidebarRadio %in% c("Regions", "All Available Breakdowns"))) {gather1 <- c(gather1, input$regionalDropDown)}
      if ((input$sidebarRadio %in% c("Equality Characteristics", "All Available Breakdowns"))) {gather1 <- c(gather1, input$equalDropDown)}
      if ((input$sidebarRadio %in% c("Employee Sizeband", "All Available Breakdowns"))) {gather1 <- c(gather1,  input$employeeDropDown)}
      if ((input$sidebarRadio %in% c("Energy type", "All Available Breakdowns"))) {gather1 <- c(gather1, input$energyDropDown)}
      if ((input$sidebarRadio %in% c("Habitat", "All Available Breakdowns"))) {gather1 <- c(gather1, input$habitatDropDown)}
      if ((input$sidebarRadio %in% c("Sector", "All Available Breakdowns"))) {gather1 <- c(gather1, input$sectorDropDown)}
      if ((input$sidebarRadio %in% c("Category", "All Available Breakdowns"))) {gather1 <- c(gather1, input$categoryDropDown)}
    }
    
    gather1 <- gather1 %>% na.omit() %>% as.character()
  ...
  output$mainPagePlot <- renderGirafe({
      if (!is.null(dataGraph())) {
        ...
        if (nrow(data1) > 0) {
          year_test <- data1 %>% pull(Year) %>% unique() %>% length()
          
          y_label <- label_info %>% 
            filter(Indicator == data1 %>% pull(Indicator) %>% unique()) %>% 
            pull(Y_Label) %>% 
            unique()
          ...
          y_higher <- data1 %>% pull(Figure) %>% min(na.rm = TRUE) < 0 ~ label_info %>% 
            filter(Indicator == data1 %>% pull(Indicator) %>% unique()) %>% 
            pull(`Higher Lim`) %>% 
            unique() %>% 
            as.numeric()
          ...
          if (!is.null(graph1)) {
            graph1 <- girafe(ggobj = graph1, 
                             width_svg = 12, 
                             height_svg = 6
            )
            graph1 <- girafe_options(graph1, 
  ...
  output$graphExplain <- renderUI({
    ...
    if (showGraph == TRUE) {
      return( {p("The below graph will show a time series if there are multiple years worth of data available or a bar chart if there is not.")
        p(paste("You can hover over the chart to get more information.",
                "The tool bar for the graph appears when hovered over the chart",
                "You can also zoom in on the graph by clicking the magnifying glass in the tool bar on the bottom left and then either double clicking or using the scroll bar on a mouse when hovered over the graph.",
                "Any changes can be reset by clicking on the button in the tool bar on the bottom left.",
                "You can also save the images as a PNG file by clicking the download button."
    ...
    if (!is.null(dataGraph())) {
      ...
      for (i in 5:ncol(dataBase)) {
        for (j in seq_len(nrow(dataBase))) {
          if (is.na(dataBase[j,i])) {dataBase[j,i] <- "-"}
        }
      }
  ...
  output$internationalGraph <- renderGirafe({
    ...
    indicator_name <- dataset %>% pull(Indicator) %>% unique()
    ...
    title <- case_when(
      indicator_name == "Spend on Research and Development" ~ paste("Scotland is in the ", tolower(scotland_ranking),
                                                                    " of OECD countries for spending on Research and Development as a % of GDP.", sep = ""),
    ...
    ## Y Max ##
    figures <- data_want[,ncol(data_want)] %>% as.numeric() %>% na.omit()
    y_max <- NA
    if (length(figures) > 0) {
