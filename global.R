#### Enterprise and Skills Dashboard Global File ####
...
#### ---------------------------------------------------------------------####
#### Reading in Measures Information (for graph labelling) ####
label_info <- read.xlsx("./Data/measureLimits.xlsx", sheet = 1, colNames = TRUE) %>%
  mutate_if(is.character, trimws)

colnames(label_info) <- gsub(".", " ", colnames(label_info), fixed = TRUE)

#### Reading in Performance Changes ####
performanceChange <- read.xlsx("./Data/Indicator Performance Changes.xlsx", na.strings = "NA")
performanceChange[is.na(performanceChange$Comparator.Rank), "Comparator.Rank"] <- ""

#### Reading in all data ####-----------------------------------------------####
#### Reading in NPF data ####
npf_data <- read.xlsx("./Data/AU_NPF_261119- Mental Wellbeing, Pay Gap, Exports (SCRIG) and Living Wage (SCRIG) added.xlsx", na.strings = c("NA", "x", "n/a", "*")) %>%
  select(Year, Figure, Breakdown, Dissaggregation, Indicator, Outcome, Measure, Source, Yearlab) %>%
  rename("Disaggregation" = Dissaggregation) %>%
  filter(Indicator %in% performanceChange$Indicator) %>%
  mutate(Year = case_when(
    ((nchar(Year) > 4) & (grepl("-", Year, fixed = TRUE) == FALSE)) ~ as.Date(as.numeric(Year), origin = "1899-12-30") %>%
      format(format = "%d %m %Y") %>%
      as.character(),
    TRUE ~ Year %>%
      as.character() %>%
  ...
  mutate(Source = Source %>% as.character() %>% trimws()) %>%
  mutate(Figure = case_when(
    Indicator == "Greenhouse gas emissions" ~ Figure * 100,
    TRUE ~ Figure
  ))

npf_data$Disaggregation <- gsub("Local authority", "Local Authority", npf_data$Disaggregation, fixed = TRUE)
npf_data$Disaggregation <- gsub("Industry Sector", "Sector", npf_data$Disaggregation, fixed = TRUE)

## Adding Data type column to npf data for easier formatting
short_list <- label_info %>%
  select(Indicator, dataType) %>%
  distinct()

npf_data <- left_join(npf_data, short_list) %>%
  mutate(Breakdown == case_when(
    Disaggregation == "Scotland" ~ "Scotland",
    TRUE ~ Breakdown
  ))

npf_data$Indicator <- gsub("International Exporting", "International exporting", npf_data$Indicator, fixed = TRUE)
npf_data$Indicator <- gsub("Employees on the Living Wage", "Employees on the Living wage", npf_data$Indicator, fixed = TRUE)

#### Reading in International Data ####
## GERD - for "Spend on Research and Development" (Economy:Productivity)##
GERD_data <- read.xlsx("./Data/GERD OECD data 2016.xlsx", sheet = 1, colNames = TRUE, na.strings = c("NA", "x"))

## TEA Rates for "Entrepreneurial Activity" (Economy:Productivity) ##
TEA_data <- read.xlsx("./Data/TEA OECD countries 2018 and 2017.xlsx", sheet = 1, colNames = TRUE, na.strings = c("NA", "x"))

## Productivity Levels data 2017 for "Productivity"##
productivity_data <- read.xlsx("./Data/Economy indicator - Productivity - Feb 19 - Data - revised.xlsx", sheet = 1, colNames = TRUE, na.strings = c("NA", "x"))
productivity_data$Country <- gsub("*", "", productivity_data$Country, fixed = TRUE)

## Equalities list to check within data making
equalities_list <- c("Age", "Disability", "Ethnicity", "Gender", "Religion", "Scottish Index of Multiple Deprivation", "Urban Rural classification")

#### Preview Graphs ####------------------------------------------------####
#### Making the Smaller Graphs that appear in the tables ####
indicator_list <- npf_data %>%
  pull(Indicator) %>%
  as.character() %>%
  unique() %>%
  sort()
...
for (i in seq_along(indicator_list)) {
  ...
  data_graph <- npf_data %>%
    ...
    filter(toupper(Indicator) == toupper(indicator_list[i])) %>%
    # filter(toupper(Breakdown) == "TOTAL") %>%
    filter(toupper(Disaggregation) == "SCOTLAND") %>%
    select(-Disaggregation)

  image_file <- indicator_list[i]
  image_file <- gsub("'", "", image_file, fixed = TRUE)

  ## Getting Upper and Lower Limits
  low_lim <- label_info %>%
    filter(toupper(Indicator) == toupper(indicator_list[i])) %>%
    pull(`Lower Lim`) %>%
    unique() %>%
  ...
  if ((length(low_lim) > 0) & (length(high_lim) > 0)) {
    if ((is.na(low_lim)) | (is.na(high_lim))) {
      low_lim <- 0
      high_lim <- 100
    }
  }

  if (nrow(data_graph) > 0) {
    ## Creating Plot
    ggplot(data = data_graph, aes(x = Year, y = Figure, group = Breakdown)) +
      geom_line(size = 2) +
      geom_point() +
      scale_y_continuous(limits = c(low_lim, high_lim)) +
      labs(x = "", y = "") +
      theme(
    ...
    ggsave(paste("./www/", image_file, ".png", sep = ""), dpi = 10)
  }
  rm(low_lim, high_lim, data_graph, image_file)
}

#### Functions ####-------------------------------------------------------####
#### Useful Functions ######
## Making a "Proper" case function for R (Upper case first character,  lower case rest)
proper_case <- function(text) {
  text <- paste0((substr(text, 1, 1) %>% enc2utf8() %>% toupper()), (substring(text, 2) %>% enc2utf8() %>% tolower()))
  return(text)
}

## Setting Formatting Rules for any tables/graphs ##
formattingFigures <- function(dataset, colnames2Format) {
  temp1 <- label_info %>%
    select(Indicator, dataType) %>%
    distinct()

  dataset1 <- left_join(dataset, temp1)

  dataset1 <- dataset1 %>%
    mutate_at(colnames2Format, function(x) {
      case_when(
        is.na(x) ~ "-",
        dataset1$dataType == "£ MILLION" ~ dollar(x, accuracy = 1, scale = 1, prefix = "£", suffix = " million"),
        dataset1$dataType == "INDEX" ~ number(x, accuracy = 0.1, scale = 1, big.mark = ","),
        dataset1$dataType == "MILLION TONNES" ~ number(x, accuracy = 0.1, scale = 1, big.mark = ",", suffix = " million tonnes"),
        dataset1$dataType == "PERCENTAGE  SMALL(%)" ~ percent(x, accuracy = 0.01, scale = 1),
        dataset1$dataType == "PERCENTAGE (%)" ~ percent(x, accuracy = 0.1, scale = 1),
        dataset1$dataType == "PERCENTAGE POINTS (%)" ~ percent(abs(x), accuracy = 0.1, scale = 1, suffix = "pp", prefix = case_when(
          x < 0 ~ "-",
          x == 0 ~ "<=>",
          x > 0 ~ "+"
        )),
        dataset1$dataType == "PERCENTAGE POINTS SMALL(%)" ~ percent(x, accuracy = 0.01, scale = 1, suffix = "pp"),
        dataset1$dataType == "PERCENTAGE WHOLE (%)" ~ percent(x, accuracy = 1, scale = 1),
        dataset1$dataType == "RANK" ~ number(x, accuracy = 1, scale = 1, big.mark = ",", suffix = case_when(
          str_sub(x, -2) == 11 ~ "th",
          str_sub(x, -2) == 12 ~ "th",
          str_sub(x, -2) == 13 ~ "th",
          str_sub(x, -1) == 1 ~ "st",
        ...
        dataset1$dataType == "SCORE" ~ number(x, accuracy = 0.1, scale = 1, big.mark = ","),
        dataset1$dataType == "VALUE" ~ number(x, accuracy = 1, scale = 1, big.mark = ",")
      )
    }) %>%
...
## Get Changes formatted to see whether a change has occured ##
formattingChanges <- function(dataset) {
  dataset1 <- left_join(dataset, performanceChange)

  dataset1 <- dataset1 %>%
    mutate(`Longer Term Trend` = case_when(
      `Long.Term.Performance` == "Improved" ~ "<img src = 'Good.png' height = '50'/>",
      `Long.Term.Performance` == "No Change" ~ "<img src = 'No Change.png' height = '50'/>",
      `Long.Term.Performance` == "Worsened" ~ "<img src = 'Bad.png' height = '50'/>",
      TRUE ~ ""
    )) %>%
    mutate(DifferenceShort = case_when(
      `Short.Term.Performance` == "Improved" ~ 1,
      `Short.Term.Performance` == "No Change" ~ 0,
      `Short.Term.Performance` == "Worsened" ~ -1
    )) %>%
    mutate(`OECD/UK Comparison` = case_when(
      ### OECD Comparators ###
      ((Wider.Performance == "Better") & (Comparator.Type == "OECD")) ~ paste0(
        "<div class = 'container'>",
        "<img src = 'Good.png' height = '50'/>",
        "<div class = 'centered'>", `Comparator.Rank` %>% trimws(), "</div>",
        "</div>"
      ),
      ((Wider.Performance == "No difference") & (Comparator.Type == "OECD")) ~ paste0(
        "<div class = 'container'>",
        "<img src = 'No Change.png' height = '50'/>",
        "<div class = 'centered'>", `Comparator.Rank` %>% trimws(), "</div>",
        "</div>"
      ),
      ((Wider.Performance == "Worse") & (Comparator.Type == "OECD")) ~ paste0(
        "<div class = 'container'>",
        "<img src = 'Bad.png' height = '50'/>",
        "<div class = 'centered'>", `Comparator.Rank` %>% trimws(), "</div>",
        "</div>"
      ),

      ### UK Comparators ###
      ((Wider.Performance == "Better") & (Comparator.Type == "UK")) ~ paste0(
        "<div class = 'container'>",
        "<img src = 'Good UK.png' height = '50'/>",
        "<div class = 'centered'>", `Comparator.Rank` %>% trimws(), "</div>",
        "</div>"
      ),
      ((Wider.Performance == "No difference") & (Comparator.Type == "UK")) ~ paste0(
        "<div class = 'container'>",
        "<img src = 'No Change UK.png' height = '50'/>",
        "<div class = 'centered'>", `Comparator.Rank` %>% trimws(), "</div>",
        "</div>"
      ),
      ((Wider.Performance == "Worse") & (Comparator.Type == "UK")) ~ paste0(
        "<div class = 'container'>",
        "<img src = 'Bad UK.png' height = '50'/>",
        "<div class = 'centered'>", `Comparator.Rank` %>% trimws(), "</div>",
        "</div>"
      ),
      TRUE ~ ""
    )) %>%
    select(colnames(dataset), `Longer Term Trend`, DifferenceShort, `OECD/UK Comparison`)
  return(dataset1)
}
# print(unique(sort(performanceChange$Wider.Performance)))
rm(i)

#### Defining Indicators ####-------------------------------------------------####
#### Defining Indicators for each of the PEWS ####
...
#### Sourcing Module Pages ####
source("./Modules/Enterprise and Skills Second Page Module.R", encoding = "UTF-8")
source("./Modules/Enterprise and Skills Third Page Module.R", encoding = "UTF-8")
...
sunburst_home_e <- read.csv("./Data/home/sunburst_home_e.csv", stringsAsFactors = FALSE)
...
growth_int <- read.csv("./Data/international/growth_int.csv", check.names = FALSE)
productivity_int <- read.csv("./Data/international/productivity_int.csv", check.names = FALSE)
exporting_int <- read.csv("./Data/international/exporting_int.csv", check.names = FALSE)
rd_int <- read.csv("./Data/international/rd_int.csv", check.names = FALSE)
entrepreneurialism_int <- read.csv("./Data/international/entrepreneurialism_int.csv", check.names = FALSE)
entrepreneurialism2_int <- read.csv("./Data/international/entrepreneurialism2_int.csv", check.names = FALSE)
depratio_int <- read.csv("./Data/international/depratio_int.csv", check.names = FALSE)
migration_int <- read.csv("./Data/international/migration_int.csv", check.names = FALSE)
eactivity_int <- read.csv("./Data/international/eactivity_int.csv", check.names = FALSE)
employment_int <- read.csv("./Data/international/employment_int.csv", check.names = FALSE)
unemployment_int <- read.csv("./Data/international/unemployment_int.csv", check.names = FALSE)
genderpaygap_int <- read.csv("./Data/international/genderpaygap_int.csv", check.names = FALSE)
genderpaygap2_int <- read.csv("./Data/international/genderpaygap2_int.csv", check.names = FALSE)
skillsunderprimary_int <- read.csv("./Data/international/skillsunderprimary_int.csv", check.names = FALSE)
skillstertiary_int <- read.csv("./Data/international/skillstertiary_int.csv", check.names = FALSE)
yunemployment_int <- read.csv("./Data/international/yunemployment_int.csv", check.names = FALSE)
evoice_int <- read.csv("./Data/international/evoice_int.csv", check.names = FALSE)
evoice2_int <- read.csv("./Data/international/evoice2_int.csv", check.names = FALSE)
lifeexpall_int <- read.csv("./Data/international/lifeexpall_int.csv", check.names = FALSE)
lifeexpf_int <- read.csv("./Data/international/lifeexpf_int.csv", check.names = FALSE)
broadband_int <- read.csv("./Data/international/broadband_int.csv", check.names = FALSE)
emissions_int <- read.csv("./Data/international/emissions_int.csv", check.names = FALSE)
ggemissions_int <- read.csv("./Data/international/ggemissions_int.csv", check.names = FALSE)
ggemissions2_int <- read.csv("./Data/international/ggemissions2_int.csv", check.names = FALSE)


# LOADING DATA - SCOTLAND ####
growth_sco <- read.csv("./Data/scotland/growth_sco.csv", check.names = FALSE)
productivity_sco <- read.csv("./Data/scotland/productivity_sco.csv", check.names = FALSE)
exporting_sco <- read.csv("./Data/scotland/exporting_sco.csv", check.names = FALSE)
exporting_destination_sco <- read.csv("./Data/scotland/exporting_destination_sco.csv", check.names = FALSE)
exporting_sector_sco <- read.csv("./Data/scotland/exporting_sector_sco.csv", check.names = FALSE)
rd_sco <- read.csv("./Data/scotland/rd_sco.csv", check.names = FALSE)
entrepreneurialism_sco <- read.csv("./Data/scotland/entrepreneurialism_sco.csv", check.names = FALSE)
reputation_sco <- read.csv("./Data/scotland/reputation_sco.csv", check.names = FALSE)
nbusiness_sector_sco <- read.csv("./Data/scotland/nbusiness_sector_sco.csv", check.names = FALSE)
nbusiness_region_sco <- read.csv("./Data/scotland/nbusiness_region_sco.csv", check.names = FALSE)
nbusiness_employees_sco <- read.csv("./Data/scotland/nbusiness_employees_sco.csv", check.names = FALSE)
hbusiness_percent_sco <- read.csv("./Data/scotland/hbusiness_percent_sco.csv", check.names = FALSE)
ibusiness_sco <- read.csv("./Data/scotland/ibusiness_sco.csv", check.names = FALSE)
rd_overview_sco <- read.csv("./Data/scotland/rd_overview_sco.csv", check.names = FALSE)
depratio_sco <- read.csv("./Data/scotland/depratio_sco.csv", check.names = FALSE)
migration_in_overseas_age_sco <- read.csv("./Data/scotland/migration_in_overseas_age_sco.csv", check.names = FALSE)
migration_in_overseas_sex_sco <- read.csv("./Data/scotland/migration_in_overseas_sex_sco.csv", check.names = FALSE)
migration_in_source_sco <- read.csv("./Data/scotland/migration_in_source_sco.csv", check.names = FALSE)
migration_type_overseas_sco <- read.csv("./Data/scotland/migration_type_overseas_sco.csv", check.names = FALSE)

eactivity_sco <- read.csv("./Data/scotland/eactivity_sco.csv", check.names = FALSE)
livingwage_sco <- read.csv("./Data/scotland/livingwage_sco.csv", check.names = FALSE)
genderpaygap_sco <- read.csv("./Data/scotland/genderpaygap_sco.csv", check.names = FALSE)
evoice_sco <- read.csv("./Data/scotland/evoice_sco.csv", check.names = FALSE)
wplearning_sco <- read.csv("./Data/scotland/wplearning_sco.csv", check.names = FALSE)
youngpplpart_sco <- read.csv("./Data/scotland/youngpplpart_sco.csv", check.names = FALSE)
skillshortage_sco <- read.csv("./Data/scotland/skillshortage_sco.csv", check.names = FALSE)

rphousingc_sco <- read.csv("./Data/scotland/rphousingc_sco.csv", check.names = FALSE)
hlifeexp_male_sco <- read.csv("./Data/scotland/hlifeexp_male_sco.csv", check.names = FALSE)
hlifeexp_female_sco <- read.csv("./Data/scotland/hlifeexp_female_sco.csv", check.names = FALSE)
mwellbeing_sco <- read.csv("./Data/scotland/mwellbeing_sco.csv", check.names = FALSE)


greenandbluespace_sco <- read.csv("./Data/scotland/greenandbluespace_sco.csv", check.names = FALSE)


cfootprint_sco <- read.csv("./Data/scotland/cfootprint_sco.csv", check.names = FALSE)
naturalf_sco <- read.csv("./Data/scotland/naturalf_sco.csv", check.names = FALSE)
gasemissions_sco <- read.csv("./Data/scotland/gasemissions_sco.csv", check.names = FALSE)


# LOADING DATA - REGIONAL ####
nbusiness_reg <- read.csv("./Data/regional/nbusiness_reg.csv", check.names = FALSE)
growth_reg <- read.csv("./Data/regional/growth_reg.csv", check.names = FALSE)
exporting_reg <- read.csv("./Data/regional/exporting_reg.csv", check.names = FALSE)
rd_reg <- read.csv("./Data/regional/rd_reg.csv", check.names = FALSE)
depratio_reg <- read.csv("./Data/regional/depratio_reg.csv", check.names = FALSE)
migration_reg <- read.csv("./Data/regional/migration_reg.csv", check.names = FALSE)
working_age_reg <- read.csv("./Data/regional/working_age_reg.csv", check.names = FALSE)
eactivity_reg <- read.csv("./Data/regional/eactivity_reg.csv", check.names = FALSE)
lwage_reg <- read.csv("./Data/regional/lwage_reg.csv", check.names = FALSE)
gpaygap_reg <- read.csv("./Data/regional/gpaygap_reg.csv", check.names = FALSE)
hlifeexp_male_reg <- read.csv("./Data/regional/hlifeexp_male_reg.csv", check.names = FALSE)
hlifeexp_female_reg <- read.csv("./Data/regional/hlifeexp_female_reg.csv", check.names = FALSE)
wplearning_reg <- read.csv("./Data/regional/wplearning_reg.csv", check.names = FALSE)
blueorgreen_reg <- read.csv("./Data/regional/blueorgreen_reg.csv", check.names = FALSE)
pubservsat_reg <- read.csv("./Data/regional/pubservsat_reg.csv", check.names = FALSE)
broadband_reg <- read.csv("./Data/regional/broadband_reg.csv", check.names = FALSE)
# LOADING DATA - EQUALITIES DASHBOARD ####
gpaygap_eq <- read.csv("./Data/equalities/gpaygap_eq.csv", check.names = FALSE)
dempgap_eq <- read.csv("./Data/equalities/dempgap_eq.csv", check.names = FALSE)
youthunemp_eq <- read.csv("./Data/equalities/youthunemp_eq.csv", check.names = FALSE)
ethnicmgap_eq <- read.csv("./Data/equalities/ethnicmgap_eq.csv", check.names = FALSE)
socioecon_eq <- read.csv("./Data/equalities/socioecon_eq.csv", check.names = FALSE)
religion_eq <- read.csv("./Data/equalities/religion_eq.csv", check.names = FALSE)
orientation_eq <- read.csv("./Data/equalities/orientation_eq.csv", check.names = FALSE)
...
# map data
mapex <- readOGR(dsn = "./Data/regional/fwlibrariesandshapefile", layer = "Scotland1")
...
growth_plain_sco <- read.csv("./Data/scotland/growth_plain_sco.csv", check.names = FALSE)
exporting_plain_sco <- read.csv("./Data/scotland/exporting_plain_sco.csv", check.names = FALSE)
rd_plain_sco <- read.csv("./Data/scotland/rd_plain_sco.csv", check.names = FALSE)
